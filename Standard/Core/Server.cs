...
namespace RemoteCore
{
    ...
    public class Server : RemoteBase
    {
        ...
        {
            this.Serialization = serial ?? RemoteBase.DefaultSerializer;
        ...
        public void NotifyClients(object message) => Parallel.ForEach(this.Clients, c => this.NotifyClient(c, message));

        /// <summary>
        /// Raws message to all of the clients.
        /// </summary>
        /// <param name="message">The message.</param>
        public void RawNotifyClients(byte[] message) => Parallel.ForEach(this.Clients, c => this.RawNotifyClient(c, message));
        ...
        private async void AwaitClients()
        {
            while (!this.Disposed)
            {
                ...
                // read the client.
                this.ClientReader(socket.Client);
            }
        ...
        {
            // reads the socket
            await this.SocketReader(client);
